...
def league_icon(lg):
    icons = {'Legend':'👑','Champion':'💎','Elite':'🔥','Diamond':'💠',
             'Platinum':'🪙','Gold':'🥇','Silver':'🥈','Bronze':'🥉','Stone':'🪨','Wood':'🪵'}
    return icons.get(lg,'')
...
for m in pool:
    ...
    try:
        ...
        print(f"failed {u}: {e}")
        enriched.append({
            'username': u, 'tier': m['tier'], 'avatar': None, 'title': None,
            'rating': None, 'peak': None, 'name': None, 'country': '',
            'league': '', 'league_icon': '', 'last_online': '',
            'followers': 0, 'wins': 0, 'draws': 0, 'losses': 0
        })
...
for m in newest_pool:
    ...
    try:
        ...
        newest.append({'username': u, 'avatar': None, 'title': None,
                       'joined': 0, 'country': '', 'league': '', 'league_icon': ''})
...
print(f"Done: {len(enriched)} active, {len(newest)} newest")
